...
# 3. 상장 종목 리스트 불러오기 함수 (로컬 저장소 활용, 서버 차단 방어)
@st.cache_data
def load_stock_list():
...
def draw_pure_zero_start_chart(df, label_name, unit_label):
    ...
    df['정렬영점누적'] = df['누적지표'] - first_day_cum # 시작점을 0으로 보정
    ...
    fig.update_yaxes(range=[-max_line, max_line], secondary_y=True, showgrid=True, title_text=f"누적 수급 ({unit_label})")
...
with tab1:
    ...
    with col1:
        selected_stock = st.selectbox("종목 선택:", stock_df['선택용_이름'])
    ...
    if len(date_range) == 2:
        df_d = get_clean_foreigner_data(ticker, date_range[0].strftime("%Y-%m-%d"), date_range[1].strftime("%Y-%m-%d"))
        if not df_d.empty:
            df_d = df_d.reset_index()
            df_d['일별지표'] = (df_d['Volume'] * df_d['Change']) / 10000
            st.plotly_chart(draw_pure_zero_start_chart(df_d, selected_stock, "만 주"), use_container_width=True)
...
with tab2:
    ...
    for stock_name in favorite_stocks:
        ...
        df_f = get_clean_foreigner_data(ticker, "2026-01-01", datetime.date.today().strftime("%Y-%m-%d"))
        if not df_f.empty:
            df_f = df_f.reset_index()
            df_f['일별지표'] = (df_f['Volume'] * df_f['Change']) / 10000
            st.plotly_chart(draw_pure_zero_start_chart(df_f, stock_name, "만 주"), use_container_width=True)
