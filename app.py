...
# 💡 [순정 복구판 수급 시각화 엔진] 실제 데이터 차트는 정직하게 그리고, 범례 아이콘만 단독 매칭
def draw_pure_zero_start_chart(df, label_name, unit_label):
    df = df.sort_values(by='Date').reset_index(drop=True)

    # 조회 기간 첫날 0점 고정 오프셋 연산
    ...
    # 1. 당일 순매수/순매도 막대그래프 렌더링 (원래 수치에 맞게 빨강/파랑으로 정직하게 표현)
    colors = ['red' if val >= 0 else 'blue' for val in df['일별지표']]

    # 실제 막대 그래프 트레이스를 범례와 연동 (더미 일체화 오류 완전 폐기)
    fig.add_trace(
        go.Bar(
            x=df['Date'],
            y=df['일별지표'],
            marker_color=colors,
            name="당일 외국인 순매매",  # 범례 텍스트 명시
            showlegend=True,  # 범례 활성화 (상단 CSS 팩에 의해 사각형 모양이 강제 적용됨)
            opacity=0.55,
    ...
    fig.update_yaxes(range=[-max_line, max_line], secondary_y=True, showgrid=True,
                     title_text=f"누적 수급 흐름 ({unit_label})")
    ...
    fig.update_layout(
        ...
        shapes=[dict(
            ...
            line=dict(color="rgba(0,0,0,0.3)", width=1.5, dash="solid")
        )]
    )
    ...
    if isinstance(date_range_1, tuple) and len(date_range_1) == 2:
        ...
        if not df_daily.empty:
            ...
            with col_stat1:
                ...
                final_zero_val = df_sorted['누적지표'].iloc[-1] - df_sorted['누적지표'].iloc[0] if not df_sorted[
                    '누적지표'].empty else 0
                st.metric("기간 내 외인 누적 증감량", f"{final_zero_val:+.2f} {hover_unit}")
            with col_stat2:
                st.metric("현재 종가 (최근 거래일)", f"{int(df_daily['Close'].iloc[-1]):,} 원",
                          delta=f"{df_daily['Change'].iloc[-1] * 100:.2f}%")
        else:
...
with tab2:
    ...
    with col_fav1:
        favorite_stocks = st.multiselect(
            "📌 즐겨찾기하여 추적할 새싹 종목들을 등록하세요:",
            options=stock_df['선택용_이름'],
            default=default_favs,
            key="fav_box"
        )

    with col_fav2:
        default_start_2 = datetime.date(2026, 1, 1)
        default_end_2 = datetime.date.today()
        date_range_2 = st.date_input(
            "📅 즐겨찾기 분석 기간을 선택하세요:", value=(default_start_2, default_end_2), min_value=datetime.date(2020, 1, 1),
            max_value=default_end_2, key="fav_date_pick"
        )

    if favorite_stocks:
        joined_str = ",".join(favorite_stocks)
        local_storage.setItem("my_sprout_favorites", joined_str)
    else:
        local_storage.setItem("my_sprout_favorites", "")

    if favorite_stocks and isinstance(date_range_2, tuple) and len(date_range_2) == 2:
        fav_start_str = date_range_2[0].strftime("%Y-%m-%d")
        fav_end_str = date_range_2[1].strftime("%Y-%m-%d")
        ...
        with st.spinner("관심 종목들의 복합 수급 지도를 갱신하는 중..."):
            for idx, stock_name in enumerate(favorite_stocks):
                ...
                df_fav = get_clean_foreigner_data(ticker, fav_start_str, fav_end_str)

                if not df_fav.empty:
                    df_fav = df_fav.reset_index()

                    if display_option == "금액 기준 (억 원)":
                        df_fav['일별지표'] = (df_fav['Close'] * df_fav['Volume'] * df_fav['Change']) / 100000000
                    else:
                        df_fav['일별지표'] = (df_fav['Volume'] * df_fav['Change']) / 10000

                    df_fav['누적지표'] = df_fav['일별지표'].cumsum()
                    ...
                    final_zero_fav = df_fav_sorted['누적지표'].iloc[-1] - df_fav_sorted['누적지표'].iloc[0] if not \
                    df_fav_sorted['누적지표'].empty else 0

                    fav_summary_list.append({
                        "종목명": name, "티커": ticker,
                        "현재가": f"{int(df_fav['Close'].iloc[-1]):,}원" if not df_fav.empty else "0원",
                        "최근 등락률": f"{df_fav['Change'].iloc[-1] * 100:.2f}%" if not df_fav.empty else "0.00%",
                        f"외인 누적 증감 ({hover_unit})": round(final_zero_fav, 2)
                    })

        if fav_summary_list:
            st.subheader("📋 즐겨찾기 새싹 수급 요약 현황판")
            summary_df = pd.DataFrame(fav_summary_list).sort_values(by=f"외인 누적 증감 ({hover_unit})",
